...
- **`src/components/Toolbar.jsx`** — the header, a **floating dock** of two stacked cards (`rounded-2xl bg-white/80 backdrop-blur-md shadow-lg`, sitting in normal flow over the `bg-slate-100` app background — not overlaying the canvas, so the blur never hits colored nodes). The **dock row** is three zones: left (logo, `CenarioSelector`, editable group name), center (the view pill `Organograma | Tabela | Cartões`), right (`+ Adicionar`, the `Ctrl+K` search trigger → palette, the focus-mode button → `F`, and the `☰` that opens the `SideDrawer`). The **filter row** holds the `FiltroMultiSelect` wrappers, the isolation chip, "Limpar (N)", the headcount metrics, and the label toggle. Admin actions (company registry, export/import, layout toggle, tour, version badge) **moved into the `SideDrawer`** — don't re-add them here. `data-tour` anchors kept in the dock: `grupo`, `adicionar`, `menu`, `filtros`.
