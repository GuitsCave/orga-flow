...
- **`src/components/Toolbar.jsx`** — the header, a **floating dock** of two stacked cards (`rounded-2xl bg-white/80 backdrop-blur-md shadow-lg`, sitting in normal flow over the `bg-slate-100` app background — not overlaying the canvas, so the blur never hits colored nodes). The **dock row** is three zones: left (logo, `CenarioSelector`, editable group name), center (the view pill `Organograma | Tabela | Cartões`), right (`+ Adicionar`, the `Ctrl+K` search trigger → palette, the focus-mode button → `F`, and the `☰` that opens the `SideDrawer`). The **filter row** is itself two always-separate lines (`flex-col`, not one `flex-wrap` row): line 1 wraps the `FiltroMultiSelect` wrappers, the isolation chip and "Limpar (N)" among themselves; line 2 (divided by a `border-t`) holds the headcount metrics and the label toggle, right-aligned. They used to share one `flex-wrap` row with the headcount pushed by `ml-auto`, which on a narrow screen could land the metrics stuck next to whichever filter chip happened to wrap last, with an ugly gap between them — forcing them onto their own row fixed that regardless of width. Admin actions (company registry, export/import, layout toggle, tour, version badge) **moved into the `SideDrawer`** — don't re-add them here. `data-tour` anchors kept in the dock: `grupo`, `adicionar`, `menu`, `filtros`.
