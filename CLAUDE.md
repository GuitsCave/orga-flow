...
**Confidential mode** (`modoConfidencial` in `App.jsx`) masks every displayed `nome` with a stable `"Colaborador N"` label — for presenting to outside consultants without exposing real names. The mapping (`rotuloAnonimoPorId`) is a `Map<id, label>` built once from the **full** `dados.pessoas` list (not the filtered one) so the same person always gets the same number across canvas/tabela/cartões/palette, and it's ordered by `nivel` + `id` only — **never by nome** — so the numbering itself can't leak a hint. `todasPessoasExibicao`/`pessoasVisiveisExibicao` are the masked copies of `dados.pessoas`/`pessoasVisiveis`; every read-only consumer (`OrgCanvas`, `TabelaView`, `CartoesView`, `CommandPalette`, `rotuloPessoa`) takes these instead of the raw lists. `PessoaForm` and anything that edits a person always uses the **real, unmasked** `dados.pessoas` — the mask is for viewers, never for the editor. `vagaAberta` people are never masked (they already have no name to hide). Toggled from the dock (`VenetianMask` icon) or the palette; view-only, never persisted or exported, like the filters. Because `PessoaNode`/`TabelaView`/`CartoesView` only render whatever `nome` they're given, none of them needed changes — masking once in `App.jsx` was enough.
...
- **`src/components/Toolbar.jsx`** — the header, a **floating dock** of two stacked cards (`rounded-2xl bg-white/80 backdrop-blur-md shadow-lg`, sitting in normal flow over the `bg-slate-100` app background — not overlaying the canvas, so the blur never hits colored nodes). The **dock row** is three zones: left (logo, `CenarioSelector`, editable group name), center (the view pill `Organograma | Tabela | Cartões`), right (`+ Adicionar`, the `Ctrl+K` search trigger → palette, the focus-mode button → `F`, and the `☰` that opens the `SideDrawer`). Dock and filter row carry explicit stacking order — `z-30` and `z-20` — strictly above `TabelaView`'s sticky `thead` (`z-10`): both cards establish their own stacking context (`relative` + `z-index`), which bubbles their *entire* subtree — dropdowns included, whatever z-index those use internally — up to that context's level for comparisons against unrelated subtrees. `header` comes before `main` in the DOM, so with **equal** z-index the sticky table header (later in the DOM) would win the tie and paint over an open filter dropdown, no matter how high that dropdown's own z-index reads locally. Keep this ordering strict (dock > filters > any in-page sticky/z-indexed content) rather than matching values — a tie is a bug waiting to resurface the moment `main`'s content changes. The **filter row** is two side-by-side columns, not one shared `flex-wrap` line: a left column (`flex-1 min-w-0`) with the `FiltroMultiSelect` wrappers, the isolation chip and "Limpar (N)" — this is the only part that wraps internally — and a right column (`shrink-0`) with the headcount metrics and the label toggle, pinned in place. They used to be flat siblings in one `flex-wrap` row with the headcount pushed by `ml-auto`; on a narrow screen, whichever filter chip wrapped last would land on the *same* line as the headcount, with an ugly gap between them. Splitting into two columns means only the filters ever wrap, and the headcount never jumps around. The filter chips also dropped their (identical, purely decorative) icon, and "Etiquetas de Empresa" only shows its text label at `2xl` — both trimmed just enough width for the common case (5 filters + headcount + toggle) to fit on one line around 1280px; it still wraps cleanly below that. Admin actions (company registry, export/import, layout toggle, tour, version badge) **moved into the `SideDrawer`** — don't re-add them here. `data-tour` anchors kept in the dock: `grupo`, `adicionar`, `menu`, `filtros`.

The app targets desktop only — the canvas itself assumes mouse pan/zoom — but both rows carry a plain `flex-wrap` (on top of the filter row's own two-column wrap) as a cheap safety net: below ~640px the dock and the headcount column drop onto extra lines instead of silently overflowing past the viewport edge. It's "less broken," not a real mobile layout; don't invest further here without an explicit ask.
- **`src/components/SideDrawer.jsx`** — the retractable right-side menu opened by the dock's `☰`. Sections: **Arquivo** (export/import JSON with inline error, plus **export PNG** — see below), **Grupo** (open `EmpresasModal`, layout auto/manual toggle, reorganize), **Ajuda** (reopen the tour, a keyboard-shortcuts list), and a footer with the app version + build date. Owns the file-input + `validarImportacao` flow that used to live in the Toolbar. Closes on `Esc`, backdrop click, and after any action that opens another surface. `App.jsx` mounts it conditionally on `drawerAberto`.
- **PNG export** (`OrgCanvas.jsx` + `SideDrawer.jsx` + `App.jsx`): `OrgCanvas` owns the export logic (`exportarComoImagem`, using `getNodesBounds`/`getViewportForBounds` from `@xyflow/react` + `html-to-image`'s `toPng` on `.react-flow__viewport`) because it's the only place with a live `useReactFlow()` instance. It registers that function with `App.jsx` via the `aoRegistrarExportacaoImagem` callback prop in a `useEffect` (registers on mount, unregisters — passes `null` — on unmount), since `OrgCanvas`/`ReactFlowProvider` only exist while `modoVisao === 'canvas'`. `App.jsx` holds the registered function in state and passes it to `SideDrawer` as `onExportarImagem`; the button there is disabled with an explanatory tooltip when it's `null` (Tabela/Cartões view). It naturally matches the filtered/isolated set and Modo Confidencial's masked names — no separate "what to export" logic needed. Two non-obvious gotchas, both hit on the first real attempt:
  - `getViewportForBounds`'s padding argument is a *fraction* of the image size when given a plain number (e.g. `0.1` = 10%) — passing a pixel value directly (e.g. `48`) silently computes a wildly wrong zoom. Pass a `"Npx"` string for an absolute pixel margin instead.
  - **Edges need an explicit inline `style.stroke`.** `html-to-image` native-clones any `<svg>` it meets (`node.cloneNode(true)`) instead of walking its children to inline each one's computed style — so a `.react-flow__edge-path` that only gets its color from React Flow's external stylesheet (the default "direct" edges, no `style` prop) exports with **no stroke at all**, even though it renders fine on screen. `paraFluxo` in `lib/layout.js` sets `style: { stroke: '#b1b1b7' }` (React Flow's own default gray) on every edge, direct or dashed, specifically so the export keeps working — don't remove it as "redundant" just because the color matches the CSS default.
  - `getNodes()` (the React Flow store) only catches up with a new filter inside the `useNodesState`/`useEffect` sync below — there's a render-cycle window where it can still hold the previous, unfiltered set. `exportarComoImagem` cross-references `getNodes()` against the current `pessoas` **prop** (always authoritative) before computing bounds, so a fast filter-then-export never exports more than what's actually filtered.
- **`src/components/Tour.jsx`** — the onboarding walkthrough (spotlight + step card). Steps live in the exported `PASSOS` array; each one points at a `data-tour="<nome>"` attribute somewhere in the UI, and a step whose target isn't in the DOM is skipped automatically. **If you move or rename a highlighted element, update its `data-tour` attribute** or that step silently disappears — this includes shipping a matching step whenever you add a genuinely new dock/drawer feature, not just wiring the anchor. The admin functions live behind the `☰`, so a single `menu` step covers them (their targets are inside the closed drawer and would otherwise be skipped). Anchors currently covered: `cenarios` (`CenarioSelector`'s root), `grupo`, `adicionar`, `canvas`, `visao` (the view pill), `buscar` (palette trigger), `confidencial`, `filtros`, `menu`. It opens on first visit (guarded by the `orga:tour-visto` localStorage key, separate from the chart data) and from the drawer's "Passo a passo".
